...
public class EndianReaders {
	...
	public static int readU8(I2CDevice device, int i2caddr, int reg, boolean verbose) throws IOException {
		...
		if (verbose) {
			System.out.println("I2C: Device " + i2caddr + " (0x" + Integer.toHexString(i2caddr) + ") returned " + result
					+ " (0x" + Integer.toHexString(result) + ") from reg " + reg + " (0x" + Integer.toHexString(reg)
					+ ")");
		}
	...
	public static int readU16(I2CDevice device, int i2caddr, int register, Endianness endianness, boolean verbose)
			throws IOException {
	...
	public static int readS16(I2CDevice device, int i2caddr, int register, Endianness endianness, boolean verbose)
			throws IOException {
		int hi = 0, lo = 0;
